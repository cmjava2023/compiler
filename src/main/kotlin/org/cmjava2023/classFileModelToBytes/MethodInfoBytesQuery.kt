...
class MethodInfoBytesQuery {
    companion object {
        fun fetch(constantPoolBuilder: ConstantPoolBuilder, methodInfo: MethodInfo): List<Byte> {
            ...
            val methodTypeIndex = constantPoolBuilder.addUtf8ConstantAndGetStartIndex(methodInfo.typeDescriptor)
            ...
            if (codeAttributeInfo != null) {
                ...
                bytesOfMethodInfo.addAll(CodeAttributeBytesQuery.fetch(constantPoolBuilder, codeAttributeInfo.code, codeAttributeNameIndex, methodInfo.typeDescriptor.content))
            } else {
